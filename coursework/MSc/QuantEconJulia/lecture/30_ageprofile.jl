...
@with_kw struct EconPars
    ...
    fs = vcat([-0.0005*(i-35)^2+1.2 for i in 1:40],fill(0.85,20))
...
"""
function natural_borrowing_limits(r,ys,T,fs)
    nat_bls = fill(0.0,T)
    minimal_income = minimum(ys)
    for t in (T-1):-1:1
        nat_bls[t] = (nat_bls[t+1]+fs[t+1]*minimal_income)/(1+r)
    end
    return nat_bls
end
...
function solve(ep::EconPars, np::NumPars)
    ...
    nat_bls = natural_borrowing_limits(r, ys, T, fs)
        ...
        for i in eachindex(cohs_at_t) # i runs through all indices of the possible cash-on-hand values
            ...
            function value_if_save_a(a) # define a function that to all candidate for end-of-period savings assigns the corresponding utility. This is the RHS of the Bellman equation assuming we save 'a'. This comes from two sources
                ...
                for yi in eachindex(ys) # add expected value from next period for each possible value of y
                    value_future = value_future + pys[yi] * vf[t+1]((1 + r) * a + fs[t+1]*ys[yi])
                end
...
function simulate(ep::EconPars, sol::Solution, N::Integer)
    ...
    consumptions = fill(0.0, N, ep.T)  # Initialize matrix for consumption values
    y_dist = DiscreteNonParametric(ep.ys, ep.pys)

    for n in 1:N
        y = rand(y_dist) * ep.fs[1]
        cohs[n, 1] = y
        incomes[n, 1] = y
    end
    ...
    for t in 2:ep.T
        for n in 1:N
            y = rand(y_dist) * ep.fs[t]
            cohs[n, t] = sol.sp[t - 1](cohs[n, t - 1]) * (1 + ep.r) + y
            incomes[n, t] = y
            consumptions[n, t-1] = sol.cp[t - 1](cohs[n, t - 1])  # Store consumption value
        end
    end

    return (cohs, incomes, consumptions)  # Return cash-on-hand, incomes, and consumptions matrices
end
...
function solve_simul(ep::EconPars, np::NumPars; N=10000)
    ...
    plot(25:84,mean(cons_sim, dims=1)', color=:blue, linewidth=2, label="consumption", xlabel = "Age")
    plot!(25:84,mean(save_sim, dims=1)', color=:red, linewidth=2, label="wealth")
    plot!(25:84,mean(y_sim, dims=1)', color=:green, linewidth=2, label="income")
end